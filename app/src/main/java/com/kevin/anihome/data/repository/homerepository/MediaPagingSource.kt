...
class MediaPagingSource
    ...
    constructor(
        ...
        override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Media> {
            ...
            val data: AniResult<List<Media>> =
                when (type) {
                    HomeTrendingTypes.TRENDING_NOW ->
                        homeRepository.getTrendingNow(
                            isAnime = isAnime,
                            page = start,
                            pageSize = pageSize,
                        )

                    HomeTrendingTypes.POPULAR_THIS_SEASON ->
                        homeRepository.getPopularThisSeason(
                            page = start,
                            pageSize = pageSize,
                        )

                    HomeTrendingTypes.UPCOMING_NEXT_SEASON ->
                        homeRepository.getUpcomingNextSeason(
                            page = start,
                            pageSize = pageSize,
                        )

                    HomeTrendingTypes.ALL_TIME_POPULAR ->
                        homeRepository.getAllTimePopularMedia(
                            isAnime = isAnime,
                            page = start,
                            pageSize = pageSize,
                        )

                    HomeTrendingTypes.TOP_100_ANIME ->
                        homeRepository.getTop100Anime(
                            isAnime = isAnime,
                            page = start,
                            pageSize = pageSize,
                        )

                    HomeTrendingTypes.POPULAR_MANHWA ->
                        homeRepository.getPopularManhwa(
                            page = start,
                            pageSize = pageSize,
                        )
                }
